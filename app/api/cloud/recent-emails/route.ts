...
function hasCloudPreview(item: Record<string, unknown>) {
  return Boolean(String(item.body_html || "").trim() || String(item.body_text || "").trim());
}

function preserveCloudPreview(item: Record<string, unknown>) {
  if (!hasCloudPreview(item)) return { ...item, preview_available: false };
  return {
    ...item,
    preview_available: true,
    preview_source: item.preview_source || "cloud_delivery_archive",
  };
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    payload.items = payload.items.slice(0, 10).map((item: Record<string, unknown>) => {
      ...
      if (!copy || !item.sent_at) return preserveCloudPreview(item);
