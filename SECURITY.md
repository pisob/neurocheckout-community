...
Community stores Cloud access and refresh tokens only inside an encrypted, HTTP-only local session cookie. Keep `NC_COMMUNITY_SESSION_SECRET` private, unique and at least 32 random characters. Internet-facing callback URLs must use HTTPS.

Subscription mutations use the dedicated `billing:write` OAuth scope and pass
through the authenticated Community server proxy. Cross-origin browser
mutations are rejected. Checkout and billing-portal return URLs are derived
from the configured OAuth callback, then validated by Cloud against the active
installation, owner and registered callback origins. Community never accepts a
browser-provided billing return URL.

Feature visibility is not authorization. Cloud recalculates entitlements and
checks scopes, account ownership, subscription state and resource ownership on
every protected API path. Community treats unknown capabilities as disabled.
Plan changes and billing failures must never erase the encrypted local vault or
its backups.
