...
> **Signed staging prerelease — v0.1.0-preview.7.** Follow the installation below
...
> **Technical Preview — v0.1.0-preview.7.** This release is intended for
...
The server-availability client is included in `v0.1.0-preview.7`, but the
...
2. Clone the current official signed release. Do not install directly from the
   ...
   ```bash
   git clone --branch v0.1.0-preview.7 --depth 1 \
...
3. Verify the release tag before running any project script:
   ...
   GNUPGHOME="${verification_home}" git verify-tag v0.1.0-preview.7
